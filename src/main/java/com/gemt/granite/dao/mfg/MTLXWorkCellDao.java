...
public class MTLXWorkCellDao {
	...
	public MTLXCUTOperBean getCutOperDetail(String jobNum) throws GraniteRestException{
		...
		try{
			log.info("Fetching CUT operations from MTLX workcell for job - " + jobNum);			
			RowMapper<MTLXCUTOperBean> rm = BeanPropertyRowMapper.newInstance(MTLXCUTOperBean.class);
			MTLXCUTOperBean operation = jdbcTemplate.queryForObject(sql, new Object[]{jobNum}, rm);
			return operation;
		}
	...
	/**
	 * Get details for all incomplete STANDARD CUT operation from MTLX work cell for all open Firm Jobs
	 * 
	 * @return
	 * @throws Exception
	 */
	public List<MTLXCUTOperBean> getMTLXCutOperations() throws GraniteRestException{
		...
		try{						
			...
			log.info("MTLX CUT Standard Operations Fetched: " + operations.size());			
			return operations;
		}
		catch(CannotGetJdbcConnectionException ex){
			ex.printStackTrace();
	...
	/**
	 * Get details for all incomplete NON-STANDARD CUT operation from MTLX work cell for all open FIRM JOBS
	 * 
	 * @return
	 * @throws Exception
	 */
	public List<MTLXCUTOperBean> getMTLNCutOperations() throws GraniteRestException{
		...
		try{						
			RowMapper<MTLXCUTOperBean> rm = BeanPropertyRowMapper.newInstance(MTLXCUTOperBean.class);
			List<MTLXCUTOperBean> operations = jdbcTemplate.query(sql, rm);
			log.info("MTLX CUT Non-Standard Operations Fetched: " + operations.size());			
			return operations;
		}
		catch(CannotGetJdbcConnectionException ex){
			ex.printStackTrace();
